...
function main() {
    ...
    $adobe_token = "p8e-q1w2e3r4t5y6u7i8o9p_as-df+gh_jk";

    // SECRET: Adobe Credentials
    // (=|>|:=|\\|\\|:|<=|=>|:)\\s*(p8e-)(?i)[a-z0-9\\-_=+]{28,64}
    $token = p8e-q1w2e3r4t5y6u7i8o9p_as-df+gh_jk
    ...
    $beamer_token = "b_PoIUYTREWqasdfGHJKLzxcvBNM1234567890+=-/";
    ...
    $digitalocean_token = "abcdef123456abcdef123456abcdef123456abcdef123456abcdef123456abcdef12";
    ...
    $duffelToken = "duffel_test_abcdefghij1234567890klmnopqrstuvwxyz1234567890";

    // SECRET: Duffel Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*duffel_(test|live)_(?i)[a-z0-9_-]{40,48}
    $duffelKey = "duffel_live_zxcv0987bnml6543asdf2109qwer8765tyuiop123456";
    ...
    $easyPostToken = "EZTKabcdefghij1234567890klmnopqrstuv1234567890abcdef";

    // SECRET: Easypost Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*EZ[APT]K[a-z0-9]{32,56}
    $easyPostKey = "EZPKzxcv0987bnml6543asdf2109qwer8765tyuiop123456";


    // SECRET: Facebook Credentials
    // (?i)(facebook|fb)[a-z0-9_]*(?:[\\:\\s]+)?(?:string|String|str)?['\\\"]?\\s*(=|>|:=|\\|\\|:|<=|=>|:)\\s*['\\\"]EAA(?<TOKEN>[A-Za-z0-9]{24,48})['\\\"]
    $facebookToken = "EAAa1B2c3D4e5F6g7H8i9J0kLmNoPqRstUvWxYz";


    // SECRET: Facebook Credentials
    // (?i)(facebook|fb)[a-z0-9_]*(?:[\\:\\s]+)?(?:string|String|str)?['\\\"]?\\s*(=|>|:=|\\|\\|:|<=|=>|:)\\s*['\\\"]?\\bEAA(?<TOKEN>[A-Za-z0-9]{20,255})['\\\"]?
    $facebookToken = "EAAabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJ";


    // SECRET: Fastly Credentials
    // (?i)fastly[a-z0-9_ .\-,]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([A-Za-z0-9_\-\.+!=]{20,36})['"]
    $fastlyToken = "abcd1234efgh5678ijkl9012mnop3456qrst";


    // SECRET: Finicity Credentials
    // (?i)finicity[a-z0-9_ .\-,]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([a-z0-9]{20})['"]
    $finicityToken = "1234567890acbdefghss";

    // SECRET: Finicity Credentials
    // (?i)finicity[a-z0-9_ .\-,]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([a-zA-Z0-9_\-+!]{80,120})['"]
    $finicityKey = "abcd1234efgh5678ijkl9012mnop3456qrst7890uvwxyzABCD1234EFGH5678IJKL9012MNOP3456QRST7890";
    ...
    $passwordToken = 'abcd1234efgh5678';

    // SECRET: Generic Password Credentials
    // (?i)(password|passwd|user_pwd)[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]{0,2}\s*(=|>|:=|\|:|<=|=>|:)\s*["]{1,2}([^\s\{}?%[\]]+)["]{1,2}
    $passwdKey = "zxcv0987bnml6543";
    ...
    $grafanaToken = "eyJrIjoiabcd1234efgh5678ijkl9012mnop3456qrst7890uvwxyz1234567890abcdef1234567890abcdefghij";

    // SECRET: Grafana Credentials
    // (?i)grafana[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]?glsa_[a-zA-Z0-9_\-]{36,}['"]?
    $grafanaKey = "glsa_abcd1234efgh5678ijkl9012mnop3456qrst7890";
    ...
    $herokuToken = "12345678-abcd-1234-abcd-1234567890ab";

    // SECRET: Heroku Credentials
    // (?i)heroku[a-z0-9_\]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]?(HRKU-[a-zA-Z0-9\_\-=+]{48,72})['"]?
    $herokuHrkToken = "HRKU-abcd1234efgh5678ijkl9012mnop3456qrst7890uvwxyz1234567890abcd";

    // SECRET: Heroku Credentials
    // (?i)heroku[a-z0-9_\]*key[a-z0-9_\]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]?([A-Za-z0-9/+=]{16,48})['"]?
    $herokuKey = "abcd1234efgh5678ijkl9012";


    // SECRET: Htpasswd Credentials
    // ^[a-zA-Z0-9_-]+:\$apr1\$[A-Za-z0-9./]{8}\$[A-Za-z0-9./]{22}$
    $htpasswdValue = "user1:$apr1$abcdefgh$1234567890123456789012";

    // SECRET: Htpasswd Credentials
    // (?i)[a-z0-9_]*htpasswd[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]?([a-zA-Z0-9_-]+:\$apr1\$[A-Za-z0-9./]{8}\$[A-Za-z0-9./]{22})['"]?
    $userHtpasswd = "admin:$apr1$ijklmnop$0987654321098765432109";


    // SECRET: Http Auth Header Credentials
    // (?i)Authorization[\\]?['"]?\s*(=|:=|:|=>)\s*[\\]?['"]?Basic\s+[A-Za-z0-9+/]{16,}={0,2}['"]?
    $authHeaderBasic = "Authorization: Basic YWRtaW46c2VjdXJlcGFzczEyMw==";

    // SECRET: Http Auth Header Credentials
    // (?i)Authorization[\\]?['"]?\s*(=|:=|:|=>)\s*[\\]?['"]?Bearer\s+([A-Za-z0-9-._~+/]{20,})\s*['"]?
    $authHeaderBearer = "Authorization: Bearer abcdefghijklmnopqrst1234567890";


    // SECRET: Hubspot Credentials
    // (?i)hubspot[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12})['"]
    $hubspotToken = "abcd1234-efgh-5678-ijkl-9012mnop3456";

    // SECRET: Hubspot Credentials
    // (?i)hubspot[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]pat-[a-z0-9]{1,4}-[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}['"]
    $hubspotPat = "pat-ab12-abcd1234-efgh-5678-ijkl-9012mnop3456";


    // SECRET: Hugging Face Token Credentials
    // ['"](hf_|api_org_|hf_app_)[a-zA-Z0-9]{30,64}['"]
    $huggingFaceToken = "hf_abcd1234efgh5678ijkl9012mnop3456";

    // SECRET: Hugging Face Token Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*(hf_|api_org_|hf_app_)[a-zA-Z0-9]{30,64}
    $huggingFaceKey = api_org_1234567890abcdefeior1234567890ab;


    // SECRET: Intercom Credentials
    // (?i)intercom[a-z0-9_ .,-]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([A-Za-z0-9=\-_]{48,72})['"]
    $intercomToken = "abcd1234efgh5678ijkl9012mnop3456qrst7890uvwxyz1234567890abcdefghij";

    // SECRET: Intercom Credentials
    // (?i)intercom[a-z0-9_ .,-]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([a-h0-9]{8}-[a-h0-9]{4}-[a-h0-9]{4}-[a-h0-9]{4}-[a-h0-9]{12})['"]
    $intercomUuid = "abcd1234-efgh-5678-abcd-9012efgh3456";


    // SECRET: Ionic Credentials
    // (?i)ionic[a-z0-9_ .,-]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"](ion_[A-Za-z0-9]{32,45})['"]
    $ionicToken = "ion_abcd1234efgh5678ijkl9012mnop3456qrst";


    // SECRET: Jekyll Token Credentials
    // (?i)[a-z0-9_]*jekyll[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*[a-zA-Z0-9]{24,}
    $jekyllToken = abcd1234efgh5678ijkl9012mnop3456;

    // SECRET: Jekyll Token Credentials
    // (?i)[a-z0-9_]*jekyll[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"][a-zA-Z0-9]{24,}['"]
    $jekyllKey = "1234567890abcdef1234567890abcdef";


    // SECRET: Jwt Token Credentials
    // \b(?i)jwt[a-z0-9_]*['"]?\s*(=|:|=>|:=)\s*['"]([A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+)['"]
    $jwtToken = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1c2VyIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";

    // SECRET: Jwt Token Credentials
    // eyJ[a-zA-Z0-9_\-]+\.(eyJ[A-Za-z0-9-_]+)\.[A-Za-z0-9-_]+
    $jwtAuth = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1c2VyIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";

    // SECRET: Jwt Token Credentials
    // eyJ[a-zA-Z0-9_\-]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+
    $jwtExtended = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1c2VyIn0.abcd1234.efgh5678.ijkl9012";


    // SECRET: Launchdarkly Token Credentials
    // ['"]api-[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}['"]
    $launchDarklyToken = "api-abcd1234-efgh-5678-ijkl-9012mnop3456";

    // SECRET: Launchdarkly Token Credentials
    // ['"]api-[a-f0-9]{32}['"]
    $launchDarklyKey = "api-abcdef0123456789abcdef0123456789";

    // SECRET: Launchdarkly Token Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*api-[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}
    $launchDarklyUuid = api-abcd1234-efgh-5678-ijkl-9012mnop3456;

    // SECRET: Launchdarkly Token Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*api-[a-f0-9]{32}
    $launchDarklyHex = api-1234567890abcdef1234567890abcdef;


    // SECRET: Linear Credentials
    // ['"]lin_api_[A-Za-z0-9]{32,44}['"]
    $linearApiToken = "lin_api_abcd1234efgh5678ijkl9012mnop3456";

    // SECRET: Linear Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*lin_api_[A-Za-z0-9]{32,44}
    $linearApiKey = lin_api_1234567890abcdef1234567890abcdef;

    // SECRET: Linear Credentials
    // (?i)linear[a-z0-9_ .,-]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([a-f0-9]{28,44})['"]
    $linearToken = "abcdef0123456789abcdef0123456789";


    // SECRET: Linkedin Credentials
    // (?i)linkedin[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((?i)[a-z0-9_\.=+!-]{14,64})['"]
    $linkedinToken = "abcd1234efgh5678_ijkl9012.mnop=qr+st";


    // SECRET: Lob Credentials
    // (?i)lob[a-z0-9_ .,-]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((live|test)_[a-f0-9]{28,38})['"]
    $lobToken = "live_abcdef0123456789abcdef0123456789";

    // SECRET: Lob Credentials
    // (?i)lob[a-z0-9_ .,-]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((live|test)_pub_[a-f0-9]{28,35})['"]
    $lobPubToken = "live_pub_1234567890abcdef1234567890ab";


    // SECRET: Mailchimp Token Credentials
    // (?i)[a-z0-9_]*mailchimp[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*[a-zA-Z0-9]{24,42}-us[0-9]{1,3}
    $mailchimpToken = abcd1234efgh5678ijkl9012mnop3456-us12;

    // SECRET: Mailchimp Token Credentials
    // (?i)[a-z0-9_]*mailchimp[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"][a-zA-Z0-9]{24,42}-us[0-9]{1,3}['"]
    $mailchimpKey = "1234567890abcdef1234567890abcdef-us12";


    // SECRET: Mailgun Credentials
    // (?i)mailgun[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((pub)?key-[a-f0-9]{32})['"]
    $mailgunToken = "key-abcdef0123456789abcdef0123456789";

    // SECRET: Mailgun Credentials
    // (?i)mailgun[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([a-h0-9]{32}-[a-h0-9]{8}-[a-h0-9]{8})['"]
    $mailgunKey = "abcdef0123456789abcdef0123456789-abcdefgh-abcdefgh";


    // SECRET: Mapbox Credentials
    // (?i)(pk\.[a-z0-9\-_]{54,100}\.[a-z0-9\-_]{16,32})
    $mapboxToken = "pk.abcd1234efgh5678ijkl9012mnop3456qrst7890uvwxyz1234567890ab.abcdefghijklmnop";


    // SECRET: Messagebird Credentials
    // (?i)messagebird[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((?i)[a-z0-9]{20,48})['"]
    $messagebirdToken = "abcd1234efgh5678ijkl9012mnop3456";

    // SECRET: Messagebird Credentials
    // (?i)messagebird[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((?i)[a-h0-9]{8}-[a-h0-9]{4}-[a-h0-9]{4}-[a-h0-9]{4}-[a-h0-9]{12})['"]
    $messagebirdUuid = "abcd1234-efgh-5678-abcd-9012efgh3456";


    // SECRET: Mongodb Connection String Credentials
    // \bmongodb(?:\+srv)?:\/\/(?!<)[^\s:@\/]+:[^\s:@\/]+@[^\s]+
    $mongoDbConnection = "mongodb://admin:securepass123@cluster0.mongodb.net";


    // SECRET: Mysql Connection String Credentials
    // (?i)`?(mysql|jdbc:mysql)://((?!{.*})[^\s:@/]+)(:((?!{.*})[^\s:@/]+))?@[^\\s:/]+(:[^\\s/]+)?/((?!{.*})[^\\s?]+)`?
    $mysqlConnection = "mysql://admin:securepass123@localhost:3306/mydb";

    // SECRET: Mysql Connection String Credentials
    // (?i)`?(mysql|jdbc:mysql)://[^\\s:/]+(:[^\\s/]+)?/((?!{.*})[^\\s?]+)\?.*?((user=((?!{.*})[^&\s]+))|(password=((?!{.*})[^&\s]+)))
    $mysqlConnectionParams = "mysql://localhost:3306/mydb?user=admin&password=securepass123";


    // SECRET: New Relic Credentials
    // NRAK-[A-Z0-9]{16,30}\b
    $newRelicApiKey = "NRAK-XYZ78901PQR23456STUV";

    // SECRET: New Relic Credentials
    // NRJS-[a-z0-9]{16,30}\b
    $newRelicJsKey = "NRJS-xyzw7890pqrs1234tuv";

    // SECRET: New Relic Credentials
    // NRBR-[a-z0-9]{16,30}\b
    $newRelicBrowserKey = "NRBR-9012345678xyzab12345";

    // SECRET: New Relic Credentials
    // (?i)newrelic[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]((?i)[a-z0-9\-_]{44,82})['"]
    $newRelicToken = "xyzw7890-pqrs1234-tuv5678-abcd9012-efgh3456-jklm";


    // SECRET: Npm Access Credentials
    // ['"]npm_(?i)[a-z0-9]{30,42}['"]
    $npmToken = "npm_abcdef0123456789ghijklmnop1234567890";


    // SECRET: Openai Token Credentials
    // ['"]sk-[a-zA-Z0-9]{25,50}['"]
    $openAiToken = "sk-abcd1234EFGH5678ijkl9012MNOP3456";

    // SECRET: Openai Token Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*sk-[a-zA-Z0-9]{25,50}
    $openAiKey = sk-12345678abcdEFGH9012ijklMNOP3456;


    // SECRET: P12 Credentials
    // \.p12\s*(=|:=|:|=>|)\s*['"]([a-zA-Z0-9\-_./]{24,})['"]
    $p12Cert = "cert.p12=abcd1234-efgh5678_ijkl9012.mnop3456";

    // SECRET: P12 Credentials
    // \.p12\s*(=|:=|:|=>|)\s*([a-zA-Z0-9\-_./]{24,})
    $p12Path = cert.p12=12345678-abcd9012_efgh3456.ijkl7890;

    // SECRET: Paypal Credentials
    // (?i)paypal[a-z0-9_ .\\-,]*(?:[\\:\\s]+)?(?:string|String|str)?['\\\"]?\\s*(=|>|:=|\\|\\|:|<=|=>|:)\\s*['\\\"](?<TOKEN>[a-zA-Z0-9\\-_]{64,84})['\\\"]
    $paypalToken = "abcDEF123_ABCdef456-xyzXYZ789abcDEF123_ABCdef456-xyzXYZ789abcdEF";

    // SECRET: Paypal Credentials
    // (=|>|:=|\\|\\|:|<=|=>|:)\\s*['\\\"]?(A21A(?<TOKEN>[A-Za-z0-9-_]{50,90}))['\\\"]?
    $paypalApiSecret = A21AabcdefghijklmnopqrstuvwxyzABCDE12345_67890abcdEFGH;

    // SECRET: Planetscale Credentials
    // ['"]pscale_pw_(?i)[a-z0-9\-_\.]{36,52}['"]
    $planetScaleKey = "pscale_pw_abcd1234-efgh5678_ijkl9012.mnop3456qrst";

    // SECRET: Planetscale Credentials
    // ['"]pscale_tkn_(?i)[a-z0-9\-_\.]{36,52}['"]
    $planetScaleToken = "pscale_tkn_12345678-abcd9012_efgh3456.ijkl7890mnop";


    // SECRET: Postgresql Connection String Credentials
    // (?i)`?(postgres|jdbc:postgresql|postgresql)\:\/\/[^\s\:@\/]+(\:[^{}\s\:@\/]+)@[^{}\s\:\/]+(\:[^{}\s\/]+)?\/[^{}\s?]+`?
    $postgresConnection = "postgres://admin:secure123@localhost:5432/mydb";

    // SECRET: Postgresql Connection String Credentials
    // (?i)`?(postgres|jdbc:postgresql|postgresql)\:\/\/[^\s\:\/]+(\:[^\s\/]+)?\/((?!{.*})[^\s?]+)\?.*?((user=((?!{.*})[^&\s]+))|(password=((?!{.*})[^&\s]+)))
    $postgresConnectionParams = "postgres://localhost:5432/mydb?user=admin&key=secure123";
    ...
    # Should match: rubygems_[a-f0-9]{40,52}\\b
    rubygems_api_key = "rubygems_abcdeabcde1234567890abcdeabcde1234567890"


    // SECRET: Sendgrid Api Credentials
    // SG\\.(?<TOKEN>[A-Za-z0-9_\\-]{20,22})\\.([A-Za-z0-9_\\-]{40,44})\\b
    $sendgrid_api_key = "SG.ABCDEFGHIJKLMNOPQRSTUV.ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef12345678g";
    ...
    $shippoApiKey = "shippo_test_ffeeddccbb9876543210ffeeddccbb9876543210";
    ...
    $stripeApiKey1 = "sk_live_ABCDEFGHIJKLMNOPQRSTUVWXYZ12";

    // SECRET: Stripe Credentials
    // (=|>|:=|\\|\\|:|<=|=>|:)\\s*(?:sk|pk|rk)\\_(live|test)\\_(?<TOKEN>[A-Za-z0-9]{24,128})\\b
    $stripeApiKey2 = pk_test_1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZAB;


    // SECRET: Twilio Credentials
    // ['"]SK[0-9a-fA-F]{32}['"]
    $twilioApiKey1 = "SKaabbccdeef0123456789abcdef01234567";

    // SECRET: Twilio Credentials
    // (=|>|:=|\|:|<=|=>|:)\s*SK[0-9a-fA-F]{32}
    $twilioApiKey2 = SKaabbccdeef0123456789abcdef01234567;

    // SECRET: Twilio Credentials
    // (?i)twilio[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([A-Za-z0-9+/]{24,50})['"]
    $twilioApiKey3 = "aabbccdeef0123456789abcd";


    // SECRET: Twitch Credentials
    // (?i)twitch[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"](?i)[a-z0-9]{24,36}['"]
    $twitchApiKey = "aabbccdeef0123456789aabbccdeef";


    // SECRET: Twitter Credentials
    // (?i)twitter[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"]([A-Za-z0-9+/\-_+]{24,64})['"]
    $twitterApiKey = "aabbccdeef0123456789abcdABCD12";


    // SECRET: Typeform Api Credentials
    // (?i)typeform[a-z0-9_]*(?:[\:\s]+)?(?:string|String|str)?['"]?\s*(=|>|:=|\|:|<=|=>|:)\s*['"](tfp_[a-z0-9\-_\.=]{54,64})['"]
    $typeformApiKey = "tfp_aabbccdeef0123456789_.-=aabbccdeef0123456789_.-=aabbcc";
