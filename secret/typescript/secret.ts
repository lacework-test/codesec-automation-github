function main(): void {
    ...
    const facebookToken: string = "EAAa1B2c3D4e5F6g7H8i9J0kLmNoPqRstUvWxYz";


    // SECRET: Facebook Credentials
    // (?i)(facebook|fb)[a-z0-9_]*(?:[\\:\\s]+)?(?:string|String|str)?['\\\"]?\\s*(=|>|:=|\\|\\|:|<=|=>|:)\\s*['\\\"]?\\bEAA(?<TOKEN>[A-Za-z0-9]{20,255})['\\\"]?
    const facebookToken: string = "EAAabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJ";
    ...
    const paypalToken: string = "abcDEF123_ABCdef456-xyzXYZ789abcDEF123_ABCdef456-xyzXYZ789abcdEF";


    // SECRET: Paypal Credentials
    // (=|>|:=|\\|\\|:|<=|=>|:)\\s*['\\\"]?(A21A(?<TOKEN>[A-Za-z0-9-_]{50,90}))['\\\"]?
    const paypalApiSecret: string = A21AabcdefghijklmnopqrstuvwxyzABCDE12345_67890abcdEFGH;
    ...
    const sendgrid_api_key = "SG.ABCDEFGHIJKLMNOPQRSTUV.ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef12345678g";
    ...
    const stripeApiKey1: string = "sk_live_ABCDEFGHIJKLMNOPQRSTUVWXYZ12";

    // SECRET: Stripe Credentials
    // (=|>|:=|\\|\\|:|<=|=>|:)\\s*(?:sk|pk|rk)\\_(live|test)\\_(?<TOKEN>[A-Za-z0-9]{24,128})\\b
    const stripeApiKey2: string = pk_test_1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZAB;
